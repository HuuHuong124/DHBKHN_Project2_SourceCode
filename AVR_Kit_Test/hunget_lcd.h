...
Cach su dung cac ham:
	...
	delay_ms(thoi_gian_tinh_theo_100us);
...
void delay_ms(unsigned int lcd4_n);
...
void LCD4_INIT(unsigned char lcd4_cur_underline, unsigned char lcd4_cur_blink)
{
	...
	delay_ms(400);						// Cho it nhat 40 ms cho LCD on dinh
	...
	for(lcd4_i=0;lcd4_i<3;lcd4_i++)
	{
		...
		lcd4_control_PORT &= ~(1<<E);	
		delay_ms(2);						// Tre it nhat 160 us de hoan thanh lenh
	}
	...
	delay_ms(41);							// Tre it nhat 4.1 ms de hoan thanh lenh

	LCD4_OUT_CMD(lcd4_cur_mode);			// Bat hien thi, va khoi tao con tro.
	delay_ms(41);							// Tre mot luc
